export default class StreamMerger {
  ...
  addStream(mediaStream, options) {
    ...
    if (this.autoMode === true) {
      ...
      videoElement.onloadedmetadata = () => {
        ...
        const pheight = mediaStream.getVideoTracks()[0].getSettings().height
        const pwidth = mediaStream.getVideoTracks()[0].getSettings().width
        if (pheight > pwidth) {
          console.debug('portrait mode')
          const xoffset = (stream.width - this.pwidth) / 2
  ...
  resizeAndSortV2() {
    ...
    let extraStreams = this.streams.length - yNumber * yNumber
    let tmp = 0
    for (let i = 1; i <= this.streams.length; i++) {
      ...
      if (extraStreams <= 0 || this.streams.length <= 3) {
        ...
        if (stream.portrait === true) {
          ...
          const xoffset = (this.vwidth - this.pwidth) / 2
          stream.x += xoffset
        } else {
