...
export default function WebRTCAdaptor(initialValues) {
  ...
  this.prepareStreamTracks = function (
    ...
    if (audioConstraint !== 'undefined' && audioConstraint !== false) {
      const mediaAudioBitrate = { audio: audioConstraint }
      navigator.mediaDevices
        .getUserMedia(mediaAudioBitrate)
  ...
  if (
    ...
    if (
          ...
          .then(function (micStream) {
            navigator.mediaDevices
              ...
              .then(function (stream) {
                ...
                const desktopSoundGainNode = audioContext.createGain()

                desktopSoundGainNode.gain.value = 1

                const audioDestionation = audioContext.createMediaStreamDestination()
      ...
      // get only audio
      const mediaAudioBitrate = { audio: thiz.mediaConstraints.audio }
      navigator.mediaDevices
        .getUserMedia(mediaAudioBitrate)
        .then(function (stream) {
          thiz.gotStream(stream)
  ...
  this.switchAudioInputSource = function (streamId, deviceId) {
    ...
    const audioTrack = thiz.localStream.getAudioTracks()[0]
    if (audioTrack) {
      audioTrack.stop()
    } else {
  ...
  this.switchVideoCameraCapture = function (streamId, deviceId) {
    // stop the track because in some android devices need to close the current camera stream
    const videoTrack = thiz.localStream.getVideoTracks()[0]
    if (videoTrack) {
      videoTrack.stop()
    } else {
  ...
  this.updateAudioTrack = function (
    ...
    if (thiz.remotePeerConnection[streamId] != null) {
      const audioTrackSender = thiz.remotePeerConnection[streamId]
        .getSenders()
        .find(function (s) {
  ...
  this.updateVideoTrack = function (
    ...
    if (thiz.remotePeerConnection[streamId] != null) {
      const videoTrackSender = thiz.remotePeerConnection[streamId]
        .getSenders()
        .find(function (s) {
    ...
    if (thiz.remotePeerConnection[streamId] == null) {
      ...
      if (dataChannelMode === 'publish') {
        ...
        if (thiz.remotePeerConnection[streamId].createDataChannel) {
          const dataChannel = thiz.remotePeerConnection[
            streamId
        ...
        if (thiz.remotePeerConnection[streamId].createDataChannel) {
          const dataChannelPeer = thiz.remotePeerConnection[
            streamId
          ].createDataChannel(streamId, dataChannelOptions)
  ...
  this.closePeerConnection = function (streamId) {
    if (thiz.remotePeerConnection[streamId] != null) {
      ...
      if (thiz.remotePeerConnection[streamId].signalingState !== 'closed') {
        ...
        const playStreamIndex = thiz.playStreamId.indexOf(streamId)
  ...
  this.muteLocalMic = function () {
    if (thiz.remotePeerConnection != null) {
      const track = thiz.localStream.getAudioTracks()[0]
      track.enabled = false
    } else {
      this.callbackError('NoActiveConnection')
    }
  ...
  this.takeConfiguration = function (
    ...
    thiz.remotePeerConnection[streamId]
      ...
      .then(function (response) {
        ...
        const length = thiz.iceCandidateList[streamId].length
  ...
  this.changeBandwidth = function (bandwidth, streamId) {
    ...
    const videoSender = thiz.getVideoSender(streamId)
  ...
  this.closeWebSocket = function () {
    for (const key in thiz.remotePeerConnection) {
  ...
  this.sendData = function (streamId, message) {
    const dataChannel = thiz.remotePeerConnection[streamId].dataChannel
    dataChannel.send(message)
  }

  function WebSocketAdaptor() {
    const wsConn = new WebSocket(thiz.websocket_url)

    let connected = false

    let pingTimerId = -1

    const clearPingTimer = function () {
      if (pingTimerId !== -1) {
        if (thiz.debug) {
